...
void FSController::fetchFrame()
{
    ...
    frame = PolyFabScanWindow::getInstance()->frameGUITransformation(frame);
    ...
    PolyFabScanWindow::getInstance()->displayFrame(result);
...
void FSController::scanThread()
{
    ...
    for(FSFloat i=0; i<360 && scanning==true; i+=stepDegrees){
        PolyFabScanWindow::getInstance()->setScanProgressCurrentDegree(i);
